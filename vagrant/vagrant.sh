...
if ! grep --quiet '^xdebug.remote_enable = on$' /etc/php/7.1/mods-available/xdebug.ini; then
    (
...
echo "GRANT ALL PRIVILEGES ON *.* TO 'vagrant'@'%' IDENTIFIED BY 'vagrant'" | mysql -uroot -proot
